...
public class GoodFateHandler {
    ...
    public static void handleGoodFate(Level level, BlockPos pos, Player player, FateLevelTypeEnum fateLevel) {
        ...
        List<IGoodFateHandler> customFates = FateEventRegistry.getGoodFates();
        if (!customFates.isEmpty()) {
            int totalWeight = customFates.stream().mapToInt(IGoodFateHandler::getWeight).sum();
            int vanillaWeight = 100; // Base weight for vanilla mod outcomes
            int roll = RANDOM.nextInt(totalWeight + vanillaWeight);
            
            if (roll < totalWeight) {
                int currentWeight = 0;
                for (IGoodFateHandler handler : customFates) {
